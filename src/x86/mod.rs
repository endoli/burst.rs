...
unsafe fn decode_group_rm(state: &mut DecodeState) {
    ...
    (*state.result).operation = GROUP_OPERATIONS[(*state.result).operation as usize][reg_field as
                                                                                         usize];
}
...
unsafe fn decode_group_0f01(state: &mut DecodeState) {
    ...
    if mod_field == 3 && reg_field != 4 && reg_field != 6 {
        (*state.result).operation = GROUP_0F01_REG_OPERATIONS[rm_field as usize][reg_field as
                                                                                     usize];
    } else {
        if reg_field < 4 {
...
unsafe fn decode_reg_group_no_operands(state: &mut DecodeState) {
    let rm_byte: u8 = read_8(state);
    (*state.result).operation =
        GROUP_OPERATIONS[(*state.result).operation as usize][(rm_byte & 7) as usize];
}
...
pub fn disassemble_to_string_16(
    ...
    if !disassemble_16(opcode, addr, max_length, instr) {
        Ok(())
    } else {
        format_instruction_string(stream, fmt, opcode, addr, instr)
    }
}
...
pub fn disassemble_to_string_32(
    ...
    if !disassemble_32(opcode, addr, max_length, instr) {
        Ok(())
    } else {
        format_instruction_string(stream, fmt, opcode, addr, instr)
    }
...
pub fn disassemble_to_string_64(
    ...
    if !disassemble_64(opcode, addr, max_length, instr) {
        Ok(())
    } else {
        format_instruction_string(stream, fmt, opcode, addr, instr)
    }
